...
def run_capture():
    ...
    with jax.set_mesh(mesh):
        print("Initializing Autoencoder with tokamax_flash (1024x1024)...")
        ...
        print("JIT Compiling and running warmup...")
        out = decode_step(dummy_z, cache)
        jax.block_until_ready(out)

        print("Starting Xprof Trace Capture...")
        with jax.profiler.trace("/tmp/xprof_trace", create_perfetto_link=False):
            start_time = time.perf_counter()
            out = decode_step(dummy_z, cache)
            jax.block_until_ready(out)
            end_time = time.perf_counter()
        
        print(f"Capture complete! The step took: {(end_time - start_time) * 1000:.2f} ms")
        print("Trace saved to /tmp/xprof_trace")
